...
days* hotDays(float** p, int d, int h, int& newCount) {
    ...
    days* pHotDays = new days[newCount];
...
int main() {
    ...
    int newCount = 0;
    days* hotdays = hotDays(data, d, h, newCount);

    std::cout << "Теплые дни:\n";

    for (int i = 0; i < newCount; ++i) {
        std::cout << "День " << (*(hotdays + i)).num << " (среднее = " << (*(hotdays + i)).temp << ")\n";
    }
